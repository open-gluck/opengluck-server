...
def test_clear_all():
    with app.test_client() as test_client:
        response = test_client.delete("/opengluck/low", headers=_headers)
        assert response.status_code == 204


def test_insert_low_1():
    test_clear_all()
    with app.test_client() as test_client:
        # insert a low
        ...
        assert response.status_code == 200
        assert response.json
        assert response.json["low-records"]["success"] is True

        response = test_client.get("/opengluck/last", headers=_headers)
        assert response.status_code == 200
        json = response.json
        assert json
        assert json["low-records"] == [
            {
                "id": "id1",
                "timestamp": timestamp,
                "sugar_in_grams": 12,
